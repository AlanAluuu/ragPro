...
def get_rag_graph(platform, model, temperature, selected_kbs, KBS):
    ...
    def call_model(state):
        ...
        # 遍历所有知识库（selected_kbs），依次查询
        context_parts = []
        for kb_name in selected_kbs:  # 注意这里要传入 selected_kbs
            retriever = get_naive_rag_tool(kb_name, as_tool=False)
            docs = retriever.get_relevant_documents(query)
            kb_context = "\n".join([f"[{kb_name}内容{i+1}]:\n{doc.page_content}" for i, doc in enumerate(docs)])
            context_parts.append(kb_context)

        context = "\n".join(context_parts)
        prompt = f"以下是一些相关知识内容：\n{context}\n\n请根据以上内容回答用户的问题：{query}"
...
def rag_chat_page():
    ...
    # 遍历知识库名称并获取工具
    for k in kbs:
        KBS[f"{k}"] = get_naive_rag_tool(k,as_tool= True)
