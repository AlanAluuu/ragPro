...
PLATFORMS = ["Ollama"] # ["fastchat"] "ZhipuAI",, "Xinference"

# 定义一个函数get_llm_models，用于获取指定平台的LLM模型列表
def get_llm_models(platform_type: Literal[tuple(PLATFORMS)], base_url: str="http://127.0.0.1:11434", api_key: str="sk-b84a908d9c6147c5ae89332722bdd0f4"):
    # 如果平台类型是Ollama
...
def get_embedding_models(platform_type: Literal[tuple(PLATFORMS)], base_url: str="http://127.0.0.1:11434", api_key: str="EMPTY"):
    # 如果平台类型是Ollama
    if platform_type == "Ollama":
        try:
            # 导入ollama模块
            ...
            # 创建Ollama客户端
            client = ollama.Client(host=base_url)
            # 获取模型列表，筛选出包含"embed"的模型
            # embedding_models = [model["model"] for model in client.list()["models"] if "bert" in model.details.families]
            # embedding_models = [model["model"] for model in client.list()["models"]]
            embedding_models = [model["model"] for model in client.list()["models"] 
                                if "nomic" in  model["model"] or "embed" in  model["model"]]
            # 返回模型列表
...
if __name__ == "__main__":
    ...
    # # 1) 先列出本地可用模型
    choices = get_embedding_models("Ollama")
    print(choices)
    # # 2) 让用户在前端选一个
    # model_name = st.selectbox("选择嵌入模型", choices)
    # # 3) 实例化选中的模型
    # embedder = get_embedding_model("Ollama", model=model_name)
    # # 4) 把用户输入文本转成向量，送去向量库或相似度计算
