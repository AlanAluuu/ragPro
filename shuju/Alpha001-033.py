...
# 计算 Alpha 因子并显示进度
# 使用 tqdm 包裹 get_alpha 函数的执行
def get_alpha_with_progress(df):
    # 显示进度条
    alphas = get_alpha(df)  # 调用现有的 get_alpha 函数
    return alphas

# 计算 Alpha 因子
calculated_factors = get_alpha_with_progress(data)

# 保存结果到 CSV 文件
calculated_factors.to_csv('calculated_alpha_factors.csv', index=False)

print("Alpha 因子计算完成，结果已保存到 'calculated_alpha_factors.csv' 文件中。")
